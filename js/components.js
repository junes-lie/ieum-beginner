(function(){
  ...
  function jCard(progress,userName="민서",{recording=false,recorded=false}={}){
    ...
    return `<aside class="separate-jcard ${recording?"is-writing":""} ${recorded?"is-recorded":""}" aria-label="카세트와 분리된 J카드">
      ...
      <div class="jcard-row"><span>TRACK 01</span>${recorded||p>=50?`<strong class="hand-a">${esc(userName)} · 낮은 파트</strong><time>9월 18일</time>`:`<em aria-label="아직 기록 없음">—</em><time>—</time>`}</div>
      <div class="jcard-row"><span>TRACK 02</span>${p>=100?'<strong class="hand-b">높은 파트</strong><time>9월 21일</time>':'<em aria-label="아직 기록 없음">—</em><time>—</time>'}</div>
    </aside>`;
  }
  ...
  function recorderCassette(status,userName="민서",imageName="02-song-duet-ribbon.webp"){
    ...
    return `<section class="recorder-cassette ${recording?"is-recording":""} ${recorded?"is-recorded":""}" aria-label="${recording?"녹음 중, 왼쪽 릴과 자기테이프가 움직이는 곡 카세트":"낮은 파트 곡 카세트"}">
