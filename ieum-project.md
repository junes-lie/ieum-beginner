...
| 믹싱 방법을 모름 | 브라우저 자동 믹싱과 원본 저장 | 결과물 완주 |
...
- 12개 논리 화면
- 마이크 기반 실시간 음고 검출, 음역 계산과 보이스 리포트
- 곡 검색·필터·파트 비교
- 열린 파트 스와이프와 키보드 대체
- 참여 조건 동의
- 실제 마이크 녹음 파일 생성·미리듣기·제출
- 작동하는 카세트 진행 상태
- 녹음과 반주를 정렬해 실제 WAV를 만드는 로컬 자동 믹싱
- 분석값 기반 음색 이미지 생성
- 음역 중첩·음색 분리도·선호 장르를 합산하는 곡·파트 궁합 계산
- localStorage 저장
...
- 서버 계정, 실제 업로드, 실시간 채팅
- 여러 기기 사이의 신청·수락·프로젝트 복제와 알림
- 실제 결제와 전문가 계약
- 외부 생성형 AI 이미지와 학습 모델 기반 AI 믹싱
- 영상 촬영의 실제 구현
- 공개 팔로워·좋아요·인기 순위
- 전문 오디오 편집
...
- 서버가 필요한 사용자 송수신은 샘플 후보 데이터로만 검증하며, 실제 기능이라고 표기하지 않는다.
- 마이크·음고·녹음·동기 반주·궁합 계산·음색 이미지·WAV 믹싱은 브라우저에서 실제로 실행한다.
